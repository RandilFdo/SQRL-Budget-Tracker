...
fun BoxWithConstraintsScope.MainScreen(screen: MainScreen) {
    ...
    // Request microphone permission when user first enters main screen
    LaunchedEffect(Unit) {
...
private fun BoxWithConstraintsScope.UI(
    ...
    androidx.compose.animation.AnimatedContent(
        ...
        transitionSpec = {
            slideInHorizontally(
                initialOffsetX = { if (targetState == MainTab.HOME) -it else it },
                animationSpec = tween(300)
            ) togetherWith slideOutHorizontally(
                targetOffsetX = { if (initialState == MainTab.HOME) it else -it },
                animationSpec = tween(300)
            )
        },
    ...
    BottomBar(
        ...
        onVoiceInput = { screen ->
            nav.navigateTo(com.ivy.navigation.VoiceInputScreen)
        },
