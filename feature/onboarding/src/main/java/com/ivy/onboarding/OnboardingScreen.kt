...
import com.ivy.onboarding.steps.OnboardingNameInput
...
import com.ivy.transaction.PermissionRequestHandler
...
private fun BoxWithConstraintsScope.UI(
    ...
    val onOfflineAccountSelected = {
        // Request microphone permission when offline account is selected
        PermissionRequestHandler.requestRecordAudioPermission(context as android.app.Activity)
        onEvent(OnboardingEvent.LoginOfflineAccount)
    }
    when (onboardingState) {
        OnboardingState.SPLASH, OnboardingState.LOGIN -> {
            OnboardingSplashLogin(
                ...
                onOfflineAccountSelected = onOfflineAccountSelected
            )
        }
        ...
        OnboardingState.ACCOUNTS -> {
            OnboardingAccounts(
                ...
                onEditAccount = { account, balance -> onEvent(OnboardingEvent.EditAccount(account, balance)) },
        ...
        OnboardingState.NAME_INPUT -> {
            OnboardingNameInput(
                onNameSet = { name -> onEvent(OnboardingEvent.SetUserName(name)) },
                onSkip = { onEvent(OnboardingEvent.SkipNameInput) }
            )
        }
    }
}
