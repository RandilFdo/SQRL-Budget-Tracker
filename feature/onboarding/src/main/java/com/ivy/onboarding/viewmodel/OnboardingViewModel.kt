...
class OnboardingViewModel @Inject constructor(
    ...
    override fun onEvent(event: OnboardingEvent) {
        viewModelScope.launch {
            when (event) {
                ...
                is OnboardingEvent.SetUserName -> setUserName(event.name)
                OnboardingEvent.SkipNameInput -> skipNameInput()
            }
        }
    }
    ...
    private suspend fun setUserName(name: String) {
        // Save the user's name to settings
        ioThread {
            settingsWriter.save(
                settingsDao.findFirst().copy(
                    name = name
        ...
        router.nameInputNext()
    }

    private suspend fun skipNameInput() {
        router.nameInputSkip()
    }
    // ---------------------------- Name Input ------------------------------------------------------
}
