...
class EditTransactionViewModel @Inject constructor(
    ...
    fun start(screen: EditTransactionScreen) {
        viewModelScope.launch {
            ...
            // Apply voice input data if provided
            applyVoiceInputData(screen)
        }
    }
    ...
    override fun onEvent(event: EditTransactionViewEvent) {
        when (event) {
            ...
            EditTransactionViewEvent.StartSpeechRecognition -> startSpeechRecognition()
            is EditTransactionViewEvent.OnSpeechRecognitionResult -> handleSpeechRecognitionResult(event.result)
    ...
    private fun onTitleChanged(newTitle: String?) {
        android.util.Log.d("VoiceInput", "=== onTitleChanged called ===")
        android.util.Log.d("VoiceInput", "New title: '$newTitle'")
        ...
        android.util.Log.d("VoiceInput", "Title state updated: '$title'")

        saveIfEditMode()

        updateTitleSuggestions(newTitle)
        
        android.util.Log.d("VoiceInput", "=== onTitleChanged complete ===")
    }
    ...
    private fun onCategoryChanged(newCategory: Category?) {
        android.util.Log.d("VoiceInput", "=== onCategoryChanged called ===")
        android.util.Log.d("VoiceInput", "New category: ${newCategory?.name?.value}")
        android.util.Log.d("VoiceInput", "Category ID: ${newCategory?.id}")
        ...
        android.util.Log.d("VoiceInput", "Category state updated: ${category?.name?.value}")

        saveIfEditMode()

        updateTitleSuggestions()
        
        android.util.Log.d("VoiceInput", "=== onCategoryChanged complete ===")
    }
    ...
    private fun startSpeechRecognition() {
        if (!speechRecognizerHelper.isSpeechRecognitionAvailable()) {
            viewModelScope.launch {
                toaster.show("Speech recognition not available on this device")
            }
            return
        }

        if (!speechRecognizerHelper.hasRecordAudioPermission()) {
            viewModelScope.launch {
                toaster.show("Microphone permission required for speech recognition")
            }
            return
        }

        // Provide available categories to the SpeechParser for better matching
        SpeechParser.setAvailableCategories(categories)

        viewModelScope.launch {
            speechRecognizerHelper.startListening()
                .onEach { result ->
                    onEvent(EditTransactionViewEvent.OnSpeechRecognitionResult(result))
                }
                .launchIn(this)
        }
    }
    ...
    private fun handleSpeechRecognitionResult(result: SpeechRecognitionResult) {
        when (result) {
            ...
            is SpeechRecognitionResult.Error -> {
                viewModelScope.launch {
                    toaster.show("Couldn't recognize speech, please try again.")
                }
            }
        }
    }

    private fun applyParsedData(parsedData: ParsedTransactionData?) {
        if (parsedData == null) return
        
        // Apply transaction type if determined
        parsedData.transactionType?.let { transactionType ->
            val ivyTransactionType = when (transactionType) {
                VoiceTransactionType.INCOME -> com.ivy.base.model.TransactionType.INCOME
                VoiceTransactionType.EXPENSE -> com.ivy.base.model.TransactionType.EXPENSE
                VoiceTransactionType.TRANSFER -> com.ivy.base.model.TransactionType.TRANSFER
            }
            onSetTransactionType(ivyTransactionType)
        }
        
        // Apply amount if found
        parsedData.amount?.let { amount ->
            onAmountChanged(amount)
        }

        // Apply date if found
        parsedData.date?.let { dateTime ->
            onDueDateChanged(dateTime)
        }

        // Apply category if found
        parsedData.category?.let { categoryName ->
            val matchingCategory = categories.find { 
                it.name.value.equals(categoryName, ignoreCase = true) 
            }
            matchingCategory?.let { category ->
                onCategoryChanged(category)
                // Set title to category name for better user experience
                onTitleChanged(category.name.value)
            }
        }

        // Apply description if found
        parsedData.description?.let { description ->
            onDescriptionChanged(description)
        }
    }

    /**
     * Applies voice input data to the transaction form when the screen starts.
     */
    private suspend fun applyVoiceInputData(screen: EditTransactionScreen) {
        android.util.Log.d("VoiceInput", "=== APPLYING VOICE INPUT DATA ===")
        android.util.Log.d("VoiceInput", "Voice amount: ${screen.voiceAmount}")
        android.util.Log.d("VoiceInput", "Voice description: ${screen.voiceDescription}")
        android.util.Log.d("VoiceInput", "Voice category: ${screen.voiceCategoryName}")
        
        // Apply voice amount if provided
        screen.voiceAmount?.let { amount ->
            android.util.Log.d("VoiceInput", "Applying amount: $amount")
            onAmountChanged(amount)
        }

        // Apply voice description if provided
        screen.voiceDescription?.let { description ->
            android.util.Log.d("VoiceInput", "Applying description: $description")
            onDescriptionChanged(description)
        }

        // Apply voice category if provided
        screen.voiceCategoryName?.let { categoryName ->
            android.util.Log.d("VoiceInput", "=== CATEGORY MATCHING ===")
            android.util.Log.d("VoiceInput", "Looking for category: '$categoryName'")
            android.util.Log.d("VoiceInput", "Available categories: ${categories.map { it.name.value }}")
            android.util.Log.d("VoiceInput", "Categories count: ${categories.size}")
            
            // Try exact match first
            var matchingCategory = categories.find { 
                it.name.value.equals(categoryName, ignoreCase = true) 
            }
            
            // If no exact match, try partial match
            if (matchingCategory == null) {
                android.util.Log.d("VoiceInput", "No exact match found, trying partial match...")
            ...
            if (matchingCategory != null) {
                android.util.Log.d("VoiceInput", "✅ Found matching category: '${matchingCategory.name.value}'")
                android.util.Log.d("VoiceInput", "Category ID: ${matchingCategory.id}")
                android.util.Log.d("VoiceInput", "Category color: ${matchingCategory.color.value}")
                
                onCategoryChanged(matchingCategory)
                // Set title to category name
                onTitleChanged(matchingCategory.name.value)
                
                android.util.Log.d("VoiceInput", "Category and title applied successfully")
            } else {
                android.util.Log.d("VoiceInput", "❌ No matching category found for: '$categoryName'")
                android.util.Log.d("VoiceInput", "Available category names: ${categories.map { "'${it.name.value}'" }}")
            }
        }
        
        android.util.Log.d("VoiceInput", "=== VOICE INPUT DATA APPLICATION COMPLETE ===")
    }
}
